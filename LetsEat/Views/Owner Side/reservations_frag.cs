...
namespace LetsEat.Views.OwnerSide
{
    public class reservations_frag : Android.Support.V4.App.Fragment
    {
        ...
        private async void DeleteReservation(string uid)
        {
            var firebase = new FirebaseClient(FBURL);
            await firebase.Child("reservations").Child(uid).DeleteAsync();
            await LoadData();
        }
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            listView.ItemClick += (s, e) =>
            {
                ...
                dequeue_btn.Visibility = ViewStates.Visible;
            };

            dequeue_btn.Click += delegate {
                if (selectedReservation != null)
                {
                    DeleteReservation(selectedReservation.uid);
                    Toast.MakeText(this.Activity, "Removed Reservation", ToastLength.Short).Show();
                }
            };
            /*
